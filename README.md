# @king3/file-kit

> FileKit — 多功能文件工具箱（Base64 转换 · 视频转音频）

[![npm version](https://img.shields.io/npm/v/@king3/file-kit.svg)](https://www.npmjs.com/package/@king3/file-kit)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

## ✨ 核心功能

- 📄 **Base64 转换**：文件 ↔ Base64 JSON，支持还原恢复
- 🎵 **视频转音频**：支持多种格式，从视频文件提取音频 (mp3 / m4a / flac / wav / alac)

## 📦 安装

### 全局安装（推荐）

```bash
pnpm install @king-3/file-kit -g
# or
npm install @king-3/file-kit -g
```

### 要求

- Node.js >= 20.0.0
- FFmpeg（视频转音频功能需要）

## 🚀 快速开始

### 命令行模式

```bash
# 查看帮助
fkt --help

# 启动交互模式（推荐新手使用）
fkt -i
```

### 交互模式

交互模式提供友好的可视化界面，一步步引导您完成操作：

```bash
fkt -i
```

---

## 📖 功能详解

### 1️⃣ Base64 转换

#### 将文件转为 Base64 JSON

将任意文件编码为 Base64 格式并保存为 JSON 文件。

**基础用法：**

```bash
fkt base64 <文件路径>
```

**示例：**

```bash
# 转换文本文件
fkt base64 document.txt

# 转换图片
fkt base64 photo.jpg

# 转换 PDF
fkt base64 report.pdf
```

**指定输出目录：**

```bash
fkt base64 file.txt -o ./output
```

**输出格式：**

```json
{
  "type": "base64",
  "createdAt": "2024-12-12 10:30:45",
  "file": {
    "name": "document.txt",
    "extension": ".txt",
    "size": 1024,
    "base64": "SGVsbG8gV29ybGQh..."
  }
}
```

---

### 2️⃣ Base64 还原

从 Base64 JSON 文件恢复原始文件。

**基础用法：**

```bash
fkt restore <JSON文件路径>
```

**示例：**

```bash
# 还原文件（默认输出到当前目录）
fkt restore archive.json

# 指定输出目录
fkt restore archive.json -o ./restored
```

**特性：**

- ✅ 自动恢复原始文件名
- ✅ 保留文件扩展名
- ✅ 显示原始创建时间

---

### 3️⃣ 视频转音频

从视频文件中提取音频轨道，支持多种格式和质量选项。

**基础用法：**

```bash
fkt v2a <视频文件路径> [选项]
# 或
fkt video-to-audio <视频文件路径> [选项]
```

#### 支持的音频格式

| 格式     | 扩展名  | 编码器     | 特点           | 适用场景        |
| -------- | ------- | ---------- | -------------- | --------------- |
| **MP3**  | `.mp3`  | libmp3lame | 通用兼容性最好 | 日常使用、分享  |
| **AAC**  | `.m4a`  | aac        | Apple 生态优化 | iOS/macOS 设备  |
| **FLAC** | `.flac` | flac       | 无损压缩       | 音乐收藏、归档  |
| **ALAC** | `.m4a`  | alac       | Apple 无损     | iTunes/iOS 无损 |
| **WAV**  | `.wav`  | pcm_s16le  | 未压缩         | 专业音频编辑    |

#### 质量选项

| 质量       | MP3  | AAC  | FLAC   | 说明         |
| ---------- | ---- | ---- | ------ | ------------ |
| **low**    | 128k | 128k | 级别 0 | 最快，体积小 |
| **medium** | 192k | 192k | 级别 5 | 平衡推荐     |
| **high**   | 320k | 256k | 级别 8 | 最佳质量     |

#### 使用示例

**1. 基础转换（使用默认设置）**

```bash
# 交互式选择格式和质量
fkt v2a video.mp4
```

**2. 指定格式**

```bash
# 转换为 MP3（高质量）
fkt v2a video.mp4 -f mp3 -q high

# 转换为 AAC（中等质量）
fkt v2a video.mp4 -f aac -q medium

# 转换为 FLAC 无损
fkt v2a video.mp4 -f flac -q high
```

**3. 指定输出路径**

```bash
# 指定输出目录
fkt v2a video.mp4 -o ./audio -f mp3 -q high

# 输出将保存为: ./audio/video.mp3
```

**4. 批量转换**

```bash
# 使用 shell 循环批量转换
for file in *.mp4; do
  fkt v2a "$file" -f mp3 -q high
done
```

#### 进度显示

转换过程中会实时显示进度：

```
⠋ 正在提取音频 45%
```

完成后显示输出路径：

```
✔ 音频已提取到: ./video.mp3
```

---

## 🎮 命令参考

### 全局选项

| 选项            | 别名 | 描述         |
| --------------- | ---- | ------------ |
| `--help`        | `-h` | 显示帮助信息 |
| `--interactive` | `-i` | 启动交互模式 |
| `--version`     | `-v` | 显示版本号   |

### base64 命令

```bash
fkt base64 <input> [选项]
```

**选项：**

- `-o, --output <path>` - 输出目录（默认：输入文件所在目录）

**别名：** 无

---

### restore 命令

```bash
fkt restore <input> [选项]
```

**选项：**

- `-o, --output <path>` - 输出目录（默认：JSON 文件所在目录）

**别名：** 无

---

### video-to-audio 命令

```bash
fkt video-to-audio <input> [选项]
fkt v2a <input> [选项]
```

**选项：**

- `-o, --output <path>` - 输出目录（默认：视频文件所在目录）
- `-f, --format <format>` - 音频格式（mp3/aac/flac/alac/wav）
- `-q, --quality <quality>` - 音频质量（low/medium/high）

**别名：** `v2a`

---

## 🎯 使用场景

### 场景 1：文件分享与存储

**问题：** 需要将二进制文件转换为文本格式，便于在 API 或数据库中存储。

**解决方案：**

```bash
# 转换为 Base64
fkt base64 document.pdf -o ./archive

# 通过 API 传输或存储到数据库
# ...

# 需要时还原
fkt restore ./archive/document.json -o ./restored
```

---

### 场景 2：视频素材音频提取

**问题：** 从视频素材中提取音频用于后期制作。

**解决方案：**

```bash
# 提取高质量 MP3
fkt v2a interview.mp4 -f mp3 -q high -o ./audio

# 提取无损 FLAC（用于专业编辑）
fkt v2a concert.mp4 -f flac -q high -o ./audio
```

---

### 场景 3：音乐归档

**问题：** 从 MV 视频中提取音乐，创建个人音乐库。

**解决方案：**

```bash
# Apple 设备用户：使用 AAC 或 ALAC
fkt v2a music_video.mp4 -f aac -q high

# 无损收藏：使用 FLAC
fkt v2a music_video.mp4 -f flac -q high
```

---

## 📊 架构设计

```
┌─────────────────────────────────────┐
│         CLI 层 (cli.ts)            │  ← Citty 命令行解析
├─────────────────────────────────────┤
│       Config 层 (config/)           │  ← 配置管理
│  • defaults.ts  (默认配置)           │
│  • audio-formats.ts  (格式配置)      │
├─────────────────────────────────────┤
│      Commands 层 (commands/)        │  ← 命令处理 + Clack 交互
│  • base64.ts                        │
│  • restore.ts                       │
│  • video-to-audio.ts                │
├─────────────────────────────────────┤
│        Core 层 (core/)              │  ← 核心业务逻辑
│  • base64-converter.ts              │
│  • video-converter.ts               │
├─────────────────────────────────────┤
│       Utils 层 (utils/)             │  ← 通用工具函数
│  • errors.ts  (错误处理)             │
│  • helpers.ts  (命令助手)            │
│  • file.ts  (文件工具)               │
│  • logger.ts  (日志输出)             │
│  • prompts.ts  (交互提示)            │
│  • time.ts  (时间格式化)              │
└─────────────────────────────────────┘
```

---

## 🛠️ 技术栈

- **CLI 框架：** [Citty](https://github.com/unjs/citty) - 现代化命令行框架
- **交互界面：** [Clack](https://github.com/natemoo-re/clack) - 美观的 CLI 提示库
- **视频处理：** [FFmpeg](https://ffmpeg.org/) - 强大的多媒体处理工具
- **样式输出：** [Ansis](https://github.com/webdiscus/ansis) - 轻量级终端颜色库

---

## ⚙️ 配置

### 环境变量

```bash
# 开启调试模式（显示详细错误堆栈）
DEBUG=1 fkt base64 file.txt
```

---

## 🔧 常见问题

### Q1: FFmpeg 相关错误

**错误：** `FFmpeg 未找到`

**解决方案：**

```bash
# macOS
brew install ffmpeg

# Ubuntu/Debian
sudo apt-get install ffmpeg

# Windows (使用 Chocolatey)
choco install ffmpeg

# 或者下载预编译版本
# https://ffmpeg.org/download.html
```

---

### Q2: 视频转音频进度卡住

**原因：** 视频文件损坏或格式不支持

**解决方案：**

1. 检查视频文件是否完整
2. 使用 `ffmpeg -i video.mp4` 测试文件是否可读
3. 尝试其他格式输出

---

### Q3: Base64 文件过大

**原因：** Base64 编码会增加约 33% 的体积

**建议：**

- 对于大文件（>10MB），考虑使用压缩工具（如 `zip`）
- Base64 更适合小型文件或需要文本传输的场景

---

### Q4: 权限错误

**错误：** `EACCES: permission denied`

**解决方案：**

```bash
# 检查文件权限
ls -la file.txt

# 修改权限
chmod 644 file.txt

# 或使用 sudo（不推荐）
sudo fkt base64 file.txt
```

---

## 📝 开发

### 本地开发

```bash
# 克隆仓库
git clone https://github.com/coderking3/file-kit.git
cd file-kit

# 安装依赖
pnpm install

# 开发模式运行
pnpm dev base64 test.txt

# 类型检查
pnpm typecheck

# 代码格式化
pnpm format

# 构建
pnpm build
```

### 项目结构

```
file-kit/
├── src/
│   ├── cli.ts              # CLI 入口
│   ├── config/             # 配置层
│   ├── commands/           # 命令层
│   ├── core/               # 核心逻辑层
│   ├── utils/              # 工具函数层
│   └── types/              # 类型定义
├── bin/
│   └── cli.mjs             # 可执行文件
├── package.json
└── README.md
```

---

## 🤝 贡献

欢迎贡献代码！请遵循以下步骤：

1. Fork 本仓库
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开启 Pull Request

---

## 📄 许可证

[MIT License](LICENSE) © 2024 King3

---

## 🔗 相关链接

- [GitHub 仓库](https://github.com/coderking3/file-kit)
- [NPM 包](https://www.npmjs.com/package/@king3/file-kit)
- [问题反馈](https://github.com/coderking3/file-kit/issues)
- [更新日志](https://github.com/coderking3/file-kit/releases)

---

## 💬 联系方式

- **作者：** King3
- **邮箱：** king3.wm@gmail.com
- **问题反馈：** [GitHub Issues](https://github.com/coderking3/file-kit/issues)

---

<div align="center">

**如果这个工具对你有帮助，请给一个 ⭐️ Star 支持一下！**

Made with ❤️ by King3

</div>
